...
dir.create(here("resources", "genotypes", "tophits", "hg19"), recursive=TRUE)
...
lapply(1:nrow(traits), \(i) {
    of <- here("resources", "genotypes", "tophits", "hg19", paste0(traits$pheno_id[i], ".txt"))
    ...
    # if(!file.exists(of)) {
        ...
        if(! "try-error" %in% class(a)) 
        {
            ...
            if(traits$pheno_id[i] == "bmi") {
                of <- here("resources", "genotypes", "tophits", "hg19", paste0("bmiz.txt"))
                write.table(a, file=of, row=F, col=F, qu=F)
            }
        }
    # }
