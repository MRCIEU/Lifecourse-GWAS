...
if(grepl("cholesterol_med", filter(df, pheno_id == phecode)$covs) == TRUE){
    analysis_data$value <- analysis_data$value + (40*analysis_data$cholesterol_med)
  }
  ...
  if(grepl("bp_med", filter(df, pheno_id == phecode)$covs) == TRUE){
   ...
   ## apply the transformation required in the phenotypes list file
        ...
        analysis_data$value <- log(analysis_data$value)
        } else if(filter(df, pheno_id == phecode)$transformation=="rank") {
         analysis_data$value <- rank(analysis_data$value, ties.method = "average")
        } else if(filter(df, pheno_id == phecode)$transformation=="none") {
         analysis_data$value <- analysis_data$value
        }
