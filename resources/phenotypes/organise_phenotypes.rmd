...
if(type == "cont" & age_range<=2){
  ...
  for(k in 1:length(cats$n)){
    ...
    pheno_out <- filter(analysis_data, (agebin == cats$agebin[k])) %>% filter(!duplicated(paste(FID, IID))) %>% select(FID, IID, value) %>% mutate(value = rank_transform(value))
  ...
  for(k in 1:length(catsls$n)){
    ...
    pheno_out <- filter(analysis_data, (lsbin == catsls$lsbin[k])) %>% filter(!duplicated(paste(FID, IID))) %>% select(FID, IID, value) %>% mutate(value = rank_transform(value))
    write.table(pheno_out, file=file.path(Sys.getenv("phenotype_processed_dir"), paste0(phecode, "_", age_group, ".phen")), row=FALSE, col=FALSE, qu=FALSE)
