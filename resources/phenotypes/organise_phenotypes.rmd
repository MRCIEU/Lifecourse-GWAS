...
for(i in 1:length(phenotypes)){
  ...
  if(type == "cont" & age_range>2){
      out <- mbw(MBW_Y=dat$value,MBW_X=dat$age, knots=5, multiple=1,lpc=0.005, upc=0.995, plotme="show", comparator=1, jitter_X=0.5, jitter_Y=0.05*y_sd)
      dat$value <- out
  }
  

if(type == "cont" & age_range<=2){
      out <- mbw(MBW_Y=dat$value,MBW_X=dat$age, knots=0, multiple=1,lpc=0.005, upc=0.995, plotme="show", comparator=1, jitter_X=0.5, jitter_Y=0.05*y_sd)
      dat$value <- out
  }
