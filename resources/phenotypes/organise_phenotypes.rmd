...
dir.create(config$phenotype_processed_dir, recursive=T)
```

# Source R functions file

```{r}
source(file.path(config$code_dir, "phenotype_functions.R"))
```
# Data input
...
```{r}
ancestry <- fread(file.path(config$genotype_input_dir, "ancestry.txt"))
names(df)
```
...
```{r}
gen_covs <- fread(file.path(config$genotype_input_dir, "gen_covariates.txt"))
names(gen_covs)
```
...
```{r}
phen_covs <- fread(file.path(config$phenotype_input_dir, "pheno_covariates.txt"))
names(phen_covs)
```
...
for(k in 1:length(ancestry_list_filtered)){
  ...
  saveRDS('pheno_out', file=file.path(config$phenotype_processed_dir, paste(phecode, paste(anc_group, paste(age_group,"pheno_dat.rds",sep="_"), sep="_"), sep = "_")))            
  sumstats <- summarise_phen(pheno_out, age_group, anc_group, male, female)
...
saveRDS(cs, file=file.path(config$results_dir, paste(phecode,"summary.rds",sep="_")))
}
}
```
